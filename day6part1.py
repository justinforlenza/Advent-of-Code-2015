...
def parse(s):
    if s.find('turn off') != -1:
        method = 0
        a = s.strip('turn off ')
    elif s.find('turn on') != -1:
        method = 1
        a = s.strip('turn on ')
    elif s.find('toggle') != -1:
        method = 2
        a = s.strip('toggle ')
    else:
        print('UH-OH')
...
def lightControl(q):
    if q[0] == 0:
        for x in range(int(q[1][0]), int(q[2][0])+1):
            for y in range(int(q[1][1]), int(q[2][1])+1):
                light_array[x][y] = -1
    elif q[0] == 1:
        for x in range(int(q[1][0]), int(q[2][0])+1):
            for y in range(int(q[1][1]), int(q[2][1])+1):
...
for instruction in instructions:
    new_instructions.append(parse(instruction))


for instruction in new_instructions:
    lightControl(instruction)


output = open('day6out', 'w')

lights = 0

for x in range(len(light_array)-1):
    for y in range(len(light_array)-1):
        if light_array[x][y] == 1:
            lights += 1
